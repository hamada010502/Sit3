...
(async () => {
  ...
  await seller.waitForURL('**/seller/pending');
  ok(await seller.getByText('Application under review').count() === 1, 'pending screen shown');
  ok(await seller.getByText('Turn on two-factor authentication').count() === 1, 'two-factor is demanded before approval');
  await shot(seller, '01-pending-2fa-required');

  step('2. Admin cannot approve until two-factor is on');
  ...
  await admin.locator('tr', { hasText: email }).getByRole('link', { name: 'View', exact: true }).click();
  await admin.getByRole('button', { name: 'Approve' }).click();
  await admin.waitForSelector('text=Turn on two-factor authentication');
  ok(true, 'approval blocked while two-factor is off');

  step('3. Seller enables two-factor');
  ...
  step('5. KYC gates payouts');
  await seller.goto(BASE + '/seller/verification');
  await seller.fill('input[name=legal_name]', 'E2E Legal Name');
  await seller.fill('input[name=national_id]', '0102030405');
  await seller.setInputFiles('input[name=doc]', upload('id.png'));
  await seller.getByRole('button', { name: 'Submit for verification' }).click();
  await seller.waitForSelector('text=Your documents are with the Paylo team');
  ok(true, 'KYC submitted');
  await admin.goto(BASE + '/admin/ops');
  ok(await admin.getByText('Identity checks waiting').count() >= 1, 'KYC appears in the operations queue');
  await shot(admin, '03-admin-ops');
  await admin.goto(BASE + '/admin/sellers?kyc=submitted');
  await admin.locator('tr', { hasText: email }).getByRole('link', { name: 'View', exact: true }).click();
  await admin.getByRole('button', { name: 'Verify seller' }).click();
  await admin.waitForSelector('span.badge:has-text("Verified")');
  ok(true, 'KYC approved');
  ...
  ok(await seen(admin, 'kyc.approved'), 'KYC decision is in the audit trail');
