  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-2xl font-bold">{t('a_disputes_title')}</h1>
        <Link href={showAll ? '/admin/disputes' : '/admin/disputes?all=1'} className="btn-secondary btn-sm">{showAll ? t('ds_open') : t('all')}</Link>
      </div>
      {rows.length === 0 && <div className="card-pad text-center text-bluewood/60">{t('none')}</div>}
      <div className="space-y-4">
        {rows.map((d) => {
          const neverLeft = (d.order.pre_dispute_status ?? d.order.status) === 'paid';
          ...
          return (
            <div key={d.id} id={d.id} className="card-pad">
              <div className="flex flex-wrap items-center gap-2 mb-2">
                ...
                <span className="font-semibold">{t('a_dispute_for')} <Link href={`/admin/orders/${d.order.id}`} className="font-mono text-crusta" dir="ltr">{d.order.code}</Link></span>
                <OrderStatusBadge status={d.order.status} t={t} />
                <span className="text-xs text-bluewood/50">{d.created_at}</span>
              </div>
              <div className="grid md:grid-cols-2 gap-4 text-sm">
                <div className="space-y-1">
                  <div><span className="text-bluewood/60">{t('dispute_reason')}: </span><strong>{t(`dr_${d.reason}` as 'dr_other')}</strong></div>
                  {d.description && <div className="text-bluewood/80 whitespace-pre-line">{d.description}</div>}
                  <div><span className="text-bluewood/60">{t('seller')}: </span>{d.store_name} · <span className="text-bluewood/60">{t('buyer')}: </span>{d.order.buyer_name} (<span dir="ltr">{d.order.buyer_phone}</span>)</div>
                  <div><span className="text-bluewood/60">{t('total')}: </span>{formatSYP(d.order.total, lang)} · <span className="text-bluewood/60">{t('fulfillment')}: </span>{t(`fm_${d.order.fulfillment_method}` as const)}{d.order.fulfillment_ref ? ` (${d.order.fulfillment_ref})` : ''}</div>
                  <div className="text-xs text-bluewood/60">{neverLeft ? t('li_seller') : d.order.handed_off_at ? `${t('hand_off_done')}: ${d.order.handed_off_at}` : ''}</div>
                ...
                {open && (
                  <div className="space-y-3">
                    ...
                    <form action={resolveAction.bind(null, d.id)} className="space-y-2 rounded-xl bg-karry/50 p-3">
                      ...
                      <p className="text-xs text-bluewood/50">{t('liability_hint')}</p>
